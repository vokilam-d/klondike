...
export class PdfGeneratorService implements OnApplicationBootstrap, OnApplicationShutdown {
  ...
  private buildTemplateContextForOrder(order: Order, lang: Language): any {
    ...
    return {
      ...
      addressName: recipient.contactInfo.middleName ? `${recipient.contactInfo.lastName} ${recipient.contactInfo.firstName} ${recipient.contactInfo.middleName}` : `${recipient.contactInfo.firstName} ${recipient.contactInfo.lastName}`,
      ...
      addressFlatNumber: recipient.address.flat,
      shipping: order.shipment.shippingMethodDescription[lang],
      shippingTip: isFreeShippingForOrder(order) ? 'бесплатная доставка' : 'оплачивается получателем',
      payment: order.paymentInfo.methodClientName[lang],
  ...
  private buildTemplateContextForInvoice(order: Order, editDto: InvoiceEditDto): any {
    ...
    return {
      ...
      addressName: editDto.addressName || (recipient.contactInfo.middleName ? `${recipient.contactInfo.lastName} ${recipient.contactInfo.firstName} ${recipient.contactInfo.middleName}` : `${recipient.contactInfo.firstName} ${recipient.contactInfo.lastName}`),
      ...
      addressFlatNumber: editDto.addressFlatNumber || recipient.address.flat,
      totalOrderCost: order.prices.totalCost,
